...
namespace Engine3D{
	...
	*/
	...
	struct circleVertex{
		...
		glm::vec4 color; // @note color the texture contains
		float thickness; // @note containing whatever texture we have (if zero then it'll sample from white texture and draw nothing)
		float fade;
	...
	*/
	struct Renderer2DData{
		...
		//! @note Batches for rendering circles
		uint32_t circleIndexCount = 0; // @note Keeping track of the base vertex buffer count
	...
	void Renderer2D::Init(){
		...
		//! @note  Circles (Rendering circles)
		...
		_data.circleVertexArray->SetIndexBuffer(quadIB); // Use Quad IB (reusing)
		...
		// _data.circleTextureShader->Set("u_Textures", samplers, _data.maxTextureSlots);
	...
	void Renderer2D::End(){
		...
		// uint32_t circleDataSize = (uint8_t *)_data.circleVertexBufferPtr - (uint8_t *)_data.circleVertexBufferBase;
		// _data.circleVertexBuffer->SetData(_data.circleVertexBufferBase, circleDataSize);
		// // }

		Flush();
		InitiateBatch(); //! @note Resets our batched rendered
	}
	...
	void Renderer2D::Flush(){
		...
		//! @note Rendering our circles. (Considering quads)
		if(_data.circleIndexCount){
			uint32_t circleDataSize = (uint8_t *)_data.circleVertexBufferPtr - (uint8_t *)_data.circleVertexBufferBase;
			_data.circleVertexBuffer->SetData(_data.circleVertexBufferBase, circleDataSize);

			_data.circleTextureShader->Bind();
			RendererCommand::drawIndexed(_data.circleVertexArray, _data.circleIndexCount);
			_data.stats.drawCalls++;
		}
	}
